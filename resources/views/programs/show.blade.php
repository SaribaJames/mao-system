...
@section('content')
    ...
    @if(!$canViewData)
        ...
        <div class="max-w-sm mx-auto bg-white rounded border border-gray-300 p-6 mt-10 text-center">
            <div class="w-14 h-14 rounded bg-primary-light border border-primary/20 flex items-center justify-center mx-auto mb-3">
            ...
            @if(session('error'))
                <div class="bg-red-50 border border-red-200 text-red-600 text-xs rounded p-2 mb-3">
                    {{ session('error') }}
                </div>
            @endif
            ...
            <form method="POST" action="{{ route('programs.unlock', $program) }}" id="pinForm">
                ...
                <div class="flex items-center justify-center gap-2 mb-5" id="pinBoxes">
                    @for ($i = 0; $i < 6; $i++)
                        <input type="password" maxlength="1" inputmode="numeric" pattern="[0-9]"
                            class="pin-box w-10 h-12 text-center text-lg font-bold border-2 border-gray-200 rounded
                ...
                <button type="submit"
                    class="w-full bg-primary hover:bg-primary-dark text-white text-sm font-semibold px-4 py-2.5 rounded transition">
                    Unlock
                </button>
            </form>
        </div>
    ...
    @else
        ...
        <div class="bg-white rounded border border-gray-300 p-4 mb-4">
            <form method="GET" class="flex gap-3">
                ...
                <button type="submit"
                    class="bg-primary text-white px-4 py-2 rounded text-sm hover:bg-primary-dark transition">
                    Search
                </button>
            </form>
        </div>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($enrollments as $enrollment)
                        <tr class="hover:bg-gray-50 transition">
                            ...
                            <td class="px-4 py-3 text-gray-600 text-xs">{{ $enrollment->processedBy?->name ?? 'â€”' }}</td>
                            <td class="px-4 py-3 text-gray-600">{{ $enrollment->remarks ?? 'â€”' }}</td>
                            <td class="px-4 py-3">
                                <span class="px-2 py-1 rounded border text-xs font-medium {{ $enrollment->status_color }}">
                                ...
                                <td class="px-4 py-3">
                                    <form method="POST" action="{{ route('program-enrollments.status', $enrollment) }}"
                                        ...
                                        <select name="status" onchange="this.form.submit()"
                                            class="border border-gray-300 rounded px-2 py-1 text-xs focus:outline-none focus:ring-2 focus:ring-primary">
        ...
        @if($pendingEndorsements->count() > 0)
            <div class="bg-yellow-50 border border-yellow-200 rounded p-5 mb-6">
                ...
                <div class="space-y-3">
                    @foreach($pendingEndorsements as $endorsement)
                        <div class="flex items-center justify-between bg-white rounded border border-yellow-200 px-4 py-3">
                            <div>
                                ...
                                <p class="text-xs text-gray-400">
                                    ...
                                    Â· {{ $endorsement->created_at->format('M d, Y') }}
                                </p>
        ...
        @if($program->id === 7)
            ...
            <div class="grid grid-cols-3 gap-4 mb-4">
                <div class="bg-white rounded border border-gray-300 p-4 text-center">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse($dispersalRecords as $record)
                            <tr class="hover:bg-gray-50 transition">
                                ...
                                <td class="px-4 py-3 text-gray-500 text-xs">{{ $record->date_received?->format('M d, Y') ?? 'â€”' }}</td>
                                <td class="px-4 py-3 text-gray-700">{{ $record->piglets_returned }}</td>
                                <td class="px-4 py-3 text-gray-500 text-xs">{{ $record->date_returned?->format('M d, Y') ?? 'â€”' }}</td>
                                <td class="px-4 py-3">
                                    <span
                                        class="px-2 py-1 rounded border text-xs font-medium
                                        {{ $record->status === 'compliant' ? 'bg-green-100 text-green-700 border-green-200' :
                                ($record->status === 'received' ? 'bg-blue-100 text-blue-700 border-blue-200' : 'bg-yellow-100 text-yellow-700 border-yellow-200') }}">
                                        {{ ucfirst($record->status) }}
                                    </span>
                                </td>
                                <td class="px-4 py-3 text-gray-500 text-xs">{{ $record->notes ?? 'â€”' }}</td>
                                @if($isAssignedUser && $isUnlocked)
                                    <td class="px-4 py-3">
                                        <button
                                ...
                                <tr class="hidden">
                                    <td>
                                        <div id="edit-dispersal-{{ $record->id }}"
                                            ...
                                            <div class="bg-white rounded border border-gray-300 w-full max-w-lg p-6">
                                                <div class="flex items-center justify-between mb-4">
                                                    <h3 class="font-semibold text-gray-800">Edit Record â€” {{ $record->farmer->first_name }}
                                                ...
                                                <form method="POST" action="{{ route('programs.dispersal.update', $record) }}">
                                                    ...
                                                    <select name="status"
                                                        class="w-full border border-gray-300 rounded px-3 py-2 text-sm mb-3 focus:outline-none focus:ring-2 focus:ring-primary">
                                                    ...
                                                    <textarea name="notes" rows="2"
                                                        class="w-full border border-gray-300 rounded px-3 py-2 text-sm mb-4 focus:outline-none focus:ring-2 focus:ring-primary">{{ $record->notes }}</textarea>
                                                    <button type="submit"
                                                        class="w-full bg-primary hover:bg-primary-dark text-white text-sm font-semibold px-4 py-2 rounded transition">
                                                        Save Changes
                                                    </button>
                                                </form>
                                            </div>
                        ...
                        <form method="POST" action="{{ route('programs.dispersal.store', $program) }}">
                            ...
                            <select name="farmer_id" required
                                ...
                                @foreach($allFarmers as $f)
                                    <option value="{{ $f->id }}">{{ $f->surname }}, {{ $f->first_name }} â€” {{ $f->barangay?->name }}
                                    </option>
                                @endforeach
                            </select>
                            <div class="grid grid-cols-2 gap-3 mb-3">
        ...
        <div class="flex items-center justify-between mb-3">
            ...
            <div class="flex items-center gap-2">
                <a href="{{ route('programs.report', $program) }}" target="_blank"
                    class="border border-gray-300 text-gray-600 hover:bg-gray-50 text-sm font-medium px-4 py-2 rounded flex items-center gap-2 transition">
            ...
            @forelse($program->activities as $activity)
                <div class="bg-white rounded border border-gray-300 p-5">
                    ...
                    <div class="grid grid-cols-2 gap-4 text-sm mb-3">
                        <div>
                            ...
                            <p class="text-gray-700">{{ $activity->performance_achieved ?: 'â€”' }}</p>
                        </div>
                        <div>
                            <p class="text-xs text-gray-400 mb-1">Challenges Encountered</p>
                            <p class="text-gray-700">{{ $activity->challenges_encountered ?: 'â€”' }}</p>
                        </div>
                        <div>
                            <p class="text-xs text-gray-400 mb-1">Proposed Intervention</p>
                            <p class="text-gray-700">{{ $activity->proposed_intervention ?: 'â€”' }}</p>
                        </div>
                        <div>
                            <p class="text-xs text-gray-400 mb-1">Target Performance</p>
                            <p class="text-gray-700">{{ $activity->target_performance ?: 'â€”' }}</p>
                        </div>
                    </div>
                            ...
                            <div class="flex flex-wrap gap-2">
                                @foreach($activity->stockUsages as $usage)
                                    <span class="px-2 py-1 bg-blue-50 text-blue-700 rounded text-xs">
                                        {{ $usage->stock->item_name }} â€” {{ $usage->quantity_used }} {{ $usage->stock->unit }}
                                    </span>
                                @endforeach
                            </div>
                        </div>
                    ...
                    <div class="border-t border-gray-100 pt-3 flex items-center justify-between">
                        <p class="text-xs text-gray-500">
                            ...
                            {{ $activity->expenditure_item ?: 'â€”' }}
                        </p>
                        <div class="flex items-center gap-3 text-xs">
                            @forelse($activity->budget_breakdown ?? [] as $year => $amount)
                                <span class="px-2 py-1 bg-gray-50 rounded text-gray-600">
                                    {{ $year }}: â‚±{{ number_format($amount, 2) }}
                                </span>
                            @empty
                ...
                @if($isAssignedUser && $isUnlocked)
                    <div id="edit-activity-modal-{{ $activity->id }}"
                        class="hidden fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
                        <div class="bg-white rounded border border-gray-300 w-full max-w-lg p-6 max-h-[90vh] overflow-y-auto">
                            <div class="flex items-center justify-between mb-4">
                            ...
                            <form method="POST" action="{{ route('programs.activities.update', $activity) }}">
                                ...
                                <textarea name="performance_achieved" rows="2"
                                    class="w-full border border-gray-300 rounded px-3 py-2 text-sm mb-3 focus:outline-none focus:ring-2 focus:ring-primary">{{ $activity->performance_achieved }}</textarea>

                                <label class="block text-xs text-gray-500 mb-1">Challenges Encountered</label>
                                <textarea name="challenges_encountered" rows="2"
                                    class="w-full border border-gray-300 rounded px-3 py-2 text-sm mb-3 focus:outline-none focus:ring-2 focus:ring-primary">{{ $activity->challenges_encountered }}</textarea>

                                <label class="block text-xs text-gray-500 mb-1">Proposed Intervention</label>
                                <textarea name="proposed_intervention" rows="2"
                                    class="w-full border border-gray-300 rounded px-3 py-2 text-sm mb-3 focus:outline-none focus:ring-2 focus:ring-primary">{{ $activity->proposed_intervention }}</textarea>

                                <label class="block text-xs text-gray-500 mb-1">Target Performance</label>
                                <textarea name="target_performance" rows="2"
                                    class="w-full border border-gray-300 rounded px-3 py-2 text-sm mb-3 focus:outline-none focus:ring-2 focus:ring-primary">{{ $activity->target_performance }}</textarea>
                                    ...
                                    @forelse($activity->budget_breakdown ?? [] as $year => $amount)
                                        <div class="flex gap-2">
                                            <input type="text" name="budget_years[]" value="{{ $year }}" placeholder="Year"
                                                class="w-24 border border-gray-300 rounded px-2 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary">
                                            <input type="text" name="budget_amounts[]" value="{{ $amount }}" placeholder="Amount"
                                                class="flex-1 border border-gray-300 rounded px-2 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary">
                                        </div>
                                    @empty
                                        <div class="flex gap-2">
                ...
                @endif
            @empty
                <div class="bg-white rounded border border-gray-300 p-8 text-center text-gray-400 text-sm">
        ...
        @if($isAssignedUser && $isUnlocked)
            <div id="enroll-modal" class="hidden fixed inset-0 bg-black/40 flex items-center justify-center z-50">
                <div class="bg-white rounded border border-gray-300 w-full max-w-md p-6">
                    <div class="flex items-center justify-between mb-4">
                        <h3 class="font-semibold text-gray-800">Enroll Farmer â€” {{ $program->name }}</h3>
            ...
            <div id="add-activity-modal" class="hidden fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
                <div class="bg-white rounded border border-gray-300 w-full max-w-5xl p-6 max-h-[90vh] overflow-y-auto">
                    <div class="flex items-center justify-between mb-4">
                        <h3 class="font-semibold text-gray-800">Add Activity â€” {{ $program->name }}</h3>
                    ...
                    <form method="POST" action="{{ route('programs.activities.store', $program) }}">
                        ...
                        <label class="block text-xs text-gray-500 mb-1">Performance Achieved</label>
                        <textarea name="performance_achieved" rows="2"
                            class="w-full border border-gray-300 rounded px-3 py-2 text-sm mb-3 focus:outline-none focus:ring-2 focus:ring-primary"></textarea>
            ...
            document.addEventListener('DOMContentLoaded', function () {
                ...
                if (years.length && document.getElementById('budgetChart')) {
                    new Chart(document.getElementById('budgetChart'), {
                        ...
                        data: {
                            labels: years,
                            datasets: [{ label: 'Total Budget (â‚±)', data: years.map(y => yearTotals[y]), borderColor: '#2563eb', backgroundColor: 'rgba(37,99,235,0.1)', fill: true, tension: 0.3 }]
                        },
