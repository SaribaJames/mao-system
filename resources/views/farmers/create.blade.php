...
<form method="POST" action="{{ route('farmers.store') }}" enctype="multipart/form-data" id="farmerForm" autocomplete="off">
@csrf
...
{{-- FIRST NAME --}}
<input type="text" name="first_name" id="mainFirstName" value="{{ old('first_name') }}" required autocomplete="off"
...
{{-- ADDRESS HOUSE --}}
<input type="text" name="house_lot_number" value="{{ old('house_lot_number') }}" autocomplete="off"
...
{{-- BARANGAY --}}
<select name="barangay_id" class="f" autocomplete="off" style="top:25.55%;left:68.38%;width:26.46%;height:1.63%;background:rgba(255,255,255,0.9);cursor:pointer;">
...
{{-- PROVINCE --}}
<input type="text" name="province" value="{{ old('province','Albay') }}" autocomplete="off"
    class="f" style="top:28.69%;left:40.18%;width:26.46%;height:1.63%;">

{{-- REGION --}}
<input type="text" name="region" value="{{ old('region') }}" autocomplete="off"
...
@for($i=0;$i<11;$i++)
<input type="text" maxlength="1" style="width:9%;height:100%;text-align:center;border:none;background:transparent;font-size:10px;font-weight:bold;padding:0;outline:none;" class="mob-d" autocomplete="off">
@endfor
</div>
...
@for($i=0;$i<10;$i++)
<input type="text" maxlength="1" style="width:10%;height:100%;text-align:center;border:none;background:transparent;font-size:10px;font-weight:bold;padding:0;outline:none;" class="land-d" autocomplete="off">
@endfor
</div>
...
<input type="checkbox" class="cb edu-cb" data-val="{{ $val }}" style="top:{{ $pos['top'] }};left:{{ $pos['left'] }};" {{ old('highest_education')==$val?'checked':'' }}>
@endforeach
...
@for($i=0;$i<8;$i++)
<input type="text" maxlength="1" style="width:12%;height:100%;text-align:center;border:none;background:transparent;font-size:10px;font-weight:bold;padding:0;outline:none;" class="dob-d" autocomplete="off">
@endfor
</div>
<input type="hidden" name="date_of_birth" id="dobH" value="{{ old('date_of_birth') }}">

{{-- PLACE OF BIRTH --}}
<input type="text" id="pob1" placeholder="City/Municipality" value="" autocomplete="off"
    class="f" style="top:35.74%;left:25.54%;width:23.42%;height:0.91%;font-size:9px;">
<input type="text" id="pob2" placeholder="Province/State" value="" autocomplete="off"
    class="f" style="top:37.03%;left:25.43%;width:11.29%;height:0.91%;font-size:9px;">
<input type="text" id="pob3" placeholder="Country" value="Philippines" autocomplete="off"
...
<input type="checkbox" class="cb" id="pwd_y" style="top:38.52%;left:75.99%;"
    onchange="if(this.checked){document.getElementById('pwd_n').checked=false;document.getElementById('pwd_h').value='1';}" {{ old('is_pwd')=='1'?'checked':'' }}>
<input type="checkbox" class="cb" id="pwd_n" style="top:38.52%;left:83.11%;"
    onchange="if(this.checked){document.getElementById('pwd_y').checked=false;document.getElementById('pwd_h').value='0';}" {{ old('is_pwd')=='0'?'checked':'' }}>
...
<input type="text" id="religionOther" name="religion_other" value="{{ old('religion_other') }}" autocomplete="off"
    class="f" style="top:40.22%;left:39.73%;width:9.61%;height:0.91%;display:{{ old('religion')=='others'?'block':'none' }};">
...
<input type="checkbox" class="cb" id="fps_y" style="top:41.09%;left:75.69%;"
    onchange="if(this.checked){document.getElementById('fps_n').checked=false;document.getElementById('fps_h').value='1';}" {{ old('is_4ps_beneficiary')=='1'?'checked':'' }}>
<input type="checkbox" class="cb" id="fps_n" style="top:41.10%;left:82.66%;"
    onchange="if(this.checked){document.getElementById('fps_y').checked=false;document.getElementById('fps_h').value='0';}" {{ old('is_4ps_beneficiary')=='0'?'checked':'' }}>
...
<input type="checkbox" class="cb" id="ind_y" style="top:42.76%;left:75.67%;"
    onchange="if(this.checked){document.getElementById('ind_n').checked=false;document.getElementById('ind_h').value='1';}" {{ old('is_indigenous')=='1'?'checked':'' }}>
<input type="checkbox" class="cb" id="ind_n" style="top:42.75%;left:82.66%;"
    onchange="if(this.checked){document.getElementById('ind_y').checked=false;document.getElementById('ind_h').value='0';}" {{ old('is_indigenous')=='0'?'checked':'' }}>
...
{{-- SPOUSE NAME --}}
<input type="text" name="spouse_name" value="{{ old('spouse_name') }}" autocomplete="off"
...
<input type="checkbox" class="cb" id="gid_y" style="top:46.62%;left:66.74%;"
    onchange="if(this.checked){document.getElementById('gid_n').checked=false;document.getElementById('gid_h').value='1';}" {{ old('has_government_id')=='1'?'checked':'' }}>
<input type="checkbox" class="cb" id="gid_n" style="top:46.62%;left:73.87%;"
    onchange="if(this.checked){document.getElementById('gid_y').checked=false;document.getElementById('gid_h').value='0';}" {{ old('has_government_id')=='0'?'checked':'' }}>
...
<input type="checkbox" class="cb" id="hh_y" style="top:50.36%;left:22.57%;"
    onchange="if(this.checked){document.getElementById('hh_n').checked=false;document.getElementById('hh_h').value='1';document.getElementById('hhSubFields').style.display='none';}" {{ old('is_household_head')=='1'?'checked':'' }}>
<input type="checkbox" class="cb" id="hh_n" style="top:50.41%;left:29.72%;"
    onchange="if(this.checked){document.getElementById('hh_y').checked=false;document.getElementById('hh_h').value='0';document.getElementById('hhSubFields').style.display='block';}" {{ old('is_household_head')=='0'?'checked':'' }}>
...
<input type="text" name="household_head_relationship" value="{{ old('household_head_relationship') }}" autocomplete="off"
    class="f" style="top:53.76%;left:24.11%;width:24.67%;height:1.39%;">
</div>
<input type="text" name="household_members_count" value="{{ old('household_members_count') }}" autocomplete="off"
...
<input type="checkbox" class="cb" id="asc_y" style="top:51.04%;left:84.31%;"
    onchange="if(this.checked){document.getElementById('asc_n').checked=false;document.getElementById('asc_h').value='1';}" {{ old('is_farmers_association_member')=='1'?'checked':'' }}>
<input type="checkbox" class="cb" id="asc_n" style="top:51.04%;left:90.27%;"
    onchange="if(this.checked){document.getElementById('asc_y').checked=false;document.getElementById('asc_h').value='0';}" {{ old('is_farmers_association_member')=='0'?'checked':'' }}>
<input type="hidden" name="is_farmers_association_member" id="asc_h" value="{{ old('is_farmers_association_member','0') }}">
<input type="text" name="farmers_association_name" value="{{ old('farmers_association_name') }}" autocomplete="off"
...
@for($i=0;$i<11;$i++)
<input type="text" maxlength="1" style="width:9%;height:100%;text-align:center;border:none;background:transparent;font-size:10px;font-weight:bold;padding:0;outline:none;" class="con-d" autocomplete="off">
@endfor
</div>
...
<input type="text" name="farming_other_crops_specify" value="{{ old('farming_other_crops_specify') }}" autocomplete="off"
...
<input type="text" name="farming_livestock_specify" value="{{ old('farming_livestock_specify') }}" autocomplete="off"
...
<input type="text" name="farming_poultry_specify" value="{{ old('farming_poultry_specify') }}" autocomplete="off"
...
<input type="text" name="farmwork_others_specify" value="{{ old('farmwork_others_specify') }}" autocomplete="off"
...
{{-- AGRI YOUTH --}}
<input type="checkbox" class="cb" name="agri_youth_farming_household" value="1" style="top:70.52%;left:79.28%;" {{ old('agri_youth_farming_household')?'checked':'' }}>
...
<input type="checkbox" class="cb" name="agri_youth_nonformal_course" value="1" style="top:73.42%;left:79.32%;" {{ old('agri_youth_nonformal_course')?'checked':'' }}>
<input type="checkbox" class="cb" name="agri_youth_participated_program" value="1" style="top:75.39%;left:79.28%;" {{ old('agri_youth_participated_program')?'checked':'' }}>
<input type="checkbox" class="cb" name="agri_youth_others" value="1" style="top:77.34%;left:79.34%;" {{ old('agri_youth_others')?'checked':'' }}>
<input type="text" name="agri_youth_others_specify" value="{{ old('agri_youth_others_specify') }}" autocomplete="off"
    class="f" style="top:77.86%;left:79.28%;width:14.72%;height:0.74%;">

{{-- GROSS ANNUAL INCOME --}}
<input type="text" name="gross_annual_income_farming" value="{{ old('gross_annual_income_farming') }}" autocomplete="off"
    class="f" style="top:79.63%;left:35.85%;width:22.18%;height:1.0%;">
<input type="text" name="gross_annual_income_non_farming" value="{{ old('gross_annual_income_non_farming') }}" autocomplete="off"
...
<input type="hidden" name="farm_location_municipality" value="{{ old('farm_location_municipality','Guinobatan') }}">
...
<input type="text" id="stubFirstName" readonly tabindex="-1"
    class="f" style="top:90.55%;left:51.26%;width:43.16%;height:1.43%;color:#333;">
<input type="text" id="stubMiddleName" readonly tabindex="-1"
    class="f" style="top:93.73%;left:4.86%;width:43.16%;height:1.43%;color:#333;">
<input type="text" id="stubExtName" readonly tabindex="-1"
    class="f" style="top:93.73%;left:50.39%;width:11.90%;height:1.43%;color:#333;">
...
<div style="width:850px;margin:0 auto 32px auto;display:flex;gap:12px;">
    ...
    <a href="{{ route('farmers.index') }}" class="border border-gray-300 text-gray-600 hover:bg-gray-50 font-medium px-8 py-2.5 rounded-md transition text-sm">
        Cancel
    </a>
</div>
